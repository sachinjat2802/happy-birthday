...
export default (parent, text) => (sketch) => {
  ...
  const fillDots = (width, height) => {
    ...
    if (width > height) {
      ...
      firstWordPoints = font.textToPoints(`${firstWord} ${secondWord}`, width * 0.06, height * 0.33, width * 0.12)
      secondWordPoints = font.textToPoints(`${thirdWord} ${fourthWord}`, width * 0.05, height * 0.83, width * 0.22);
    } else {
      ...
      thirdWordPoints = font.textToPoints(thirdWord, width * 0.01, height * 0.3, width * 0.3);
      fourthWordPoints = font.textToPoints(fourthWord, width * 0.01, height * 0.5, width * 0.25);
    }
